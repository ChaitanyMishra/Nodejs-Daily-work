...
const port = 3001;
const  fs = require("fs")
app.use(express.urlencoded({extended : false}),(req,res) =>{
    next();
})
app.use((req,res,next)=>{
    fs.appendFile("log.txt" , `User Request at ${Date.now().toLocaleString()} IP Address: ${req.ip} Method: ${req.method}` ,(err)=>{
...
app.post("/api/users" , (req , res)=> {
    ...
    fs.writeFile("./MOCK_DATA.json" , JSON.stringify(users) , (err)=>{res.send("error" , err)})
    return res.json({status : "Sucessful"})
