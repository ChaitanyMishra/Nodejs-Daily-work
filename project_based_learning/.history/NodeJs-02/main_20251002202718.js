...
const server = http.createServer((req, res) => {
    ...
    if (pathname === "/") {
        ...
        fs.readFile("server.json", "utf-8", (err, data) => {
            ...
            // Check query params
            if (pathname === "/about") {
                if (query.name) return res.end(`Name: ${jsonData.about.name}`);
                if (query.course) return res.end(`Course: ${jsonData.about.course}`);
                if (query.email) return res.end(`Email: ${jsonData.about.email}`);
                if (query.cgpa) return res.end(`Email: ${jsonData.about.cgpa}`);
                if (query.sem) return res.end(`Email: ${jsonData.about.semester}`);
                if (query.emai) return res.end(`Email: ${jsonData.about.email}`);
